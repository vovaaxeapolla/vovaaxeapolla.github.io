...
async function login(consoleStore: ConsoleStore) {
    ...
    typingAnim('Введите ваш логин', consoleStore.linesStore.write.bind(consoleStore.linesStore))
    ...
    consoleStore.linesStore.writeln(new LineData(LineType.TEXT, ''));
    typingAnim('Введите ваш пароль', consoleStore.linesStore.write.bind(consoleStore.linesStore))
    const password: string = await AsyncInput<string>();
    consoleStore.linesStore.writeln(new LineData(LineType.TEXT, '*'.repeat(password.length)));
...
async function signup(consoleStore: ConsoleStore) {
    consoleStore.isBusy = true;
    consoleStore.linesStore.writeln(new LineData(LineType.TEXT, ''));
    typingAnim('Введите ваш email', consoleStore.linesStore.write.bind(consoleStore.linesStore))
...
async function profile(profile: string) {
    const data = await fetch(`https://fakerapi.it/api/v1/users?_quantity=1&_gender=male`);
    ...
    //     </span>
    // )))
    return new LineData(LineType.TEXT, <img src="https://fikiwiki.com/uploads/posts/2022-02/1644965605_9-fikiwiki-com-p-kartinki-priroda-na-zastavku-telefona-9.jpg" />)
}

const account = {
    fn: async function ({ consoleStore, args }: ICommand) {
        if (args[0] === 'login') {
            login(consoleStore);
            return;
        }
