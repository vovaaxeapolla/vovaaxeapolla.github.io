...
const CMDFunctions: IModule = {
        ...
        fn: ({ args, texts }: ICommand) => {
            if (args.length === 0) {
                ...
                if (todos && todos.length > 0)
                    return todos.map((td: string, i: number) => new LineData(LineType.TEXT, i + 1 + ": " + td));
                else
            ...
            if (args[0] === 'remove' && texts[0]) {
                ...
                if (!Array.isArray(todos))
                    return new LineData(LineType.TEXT, 'Список дел пуст');
                let newTodos = todos.filter((v, i) => i !== +texts[0] - 1);
    ...
    weather: {
        fn: async ({ args, texts }: ICommand) => {
            ...
            function generateTable(title: string, array: [][]) {
                const maxWidth = 73;
                ...
                return rows.map(row => row.reduce((p: string, c: string | [string, number]): string => {
                    if (typeof c === 'string') {
                        return p + c;
                    } else {
